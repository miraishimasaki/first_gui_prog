...
def data_clear():
    ...
    url_var.set(value='')
    try:
        page_var.set(value=5)
        piece_var.set(value=1)
...
def baidu_scapy():
    global url,page,path,piece,number
    pop = True
    if url == '':
        pop = False
...
def pop_congratulations():
    messagebox.showinfo(title='Congratulations',message='爬取完成!此次爬取中没有出现任何错误！')
